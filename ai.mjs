...
setInterval(() => fetch(LUMEN_PING_URL).then(res => console.log('Lumen ping', res.status)).catch(err => console.error(err)), 10 * 60 * 1000);
...
app.post('/ask', async (req, res) => {
    try {
        ...
        // IMAGE generation
        if (type === 'image') {
            if (userTier !== 'premium' && userTier !== 'ultra') return res.status(403).json({ error: 'Image generation only for premium users.' });
        ...
        if (fileUrl) {
            ...
            if (isImage) {
                if (!['gpt-4o', 'gpt-5'].includes(chatModel)) {
                    return res.status(400).json({ error: 'Image analysis requires Lumen o3/V.' });
                }
                userMessageContent = [
