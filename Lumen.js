...
async function response(userInput) {
    try {
        // Prepare system prompt (keep yours or tweak as needed)
        ...
        const res = await fetch('https://lumen-ai.onrender.com/ask', {
            ...
            body: JSON.stringify({
                prompt: userInput,
                system: systemPrompt,
                model: lumenUser.premium ? 'Premium' : 'Free'
            })
        });
        ...
        if (!reply) {
            reply = "Oops, no response from Lumen. Try again!";
            console.warn('Empty reply from API');
        }
        ...
        newMessage.textContent = 'Lumen: ' + reply;

        // Append to chat container
        const container = document.getElementById(`a${messages}a`);
        if (container) {
            container.appendChild(newMessage);
        } else {
            console.error(`Container with id a${messages}a not found`);
            alert('Chat container missing, check console for errors.');
        }

    } catch (error) {
        console.error('Error in response():', error);
        alert('There was an error communicating with Lumen. Check console.');
    }
}
